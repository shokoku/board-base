...
<div class="container">
    <form:form action="/board/${board.id}/edit"  method="get">
        ...
        <div class="mb-3">
            <input type="checkbox" class="form-check-input" id="secret" name="secret" ${board.secret ? 'checked' : ''} disabled>
            <label class="form-check-label" for="secret">비밀글 설정</label>
        </div>

        <div class="mb-3">
            <label>중요도</label><br>
            <input type="radio" name="importance" value="LOW" id="low" ${board.importance == 'LOW' ? 'checked' : ''} disabled>
            <label for="low">낮음</label>

            <input type="radio" name="importance" value="MEDIUM" id="medium" ${board.importance == 'MEDIUM' ? 'checked' : ''} disabled>
            <label for="medium">보통</label>

            <input type="radio" name="importance" value="HIGH" id="high" ${board.importance == 'HIGH' ? 'checked' : ''} disabled>
